...
class Hybrid_LISTA_cpss (LISTA_base):
    ...
    def __init__(self, A, T, lam, percent, max_percent,
        ...
        self._ps_dnn = [(t+1) * self._p / 2 for t in range (self._T)]
        self._ps_dnn = np.clip (self._ps_dnn, 0.0, self._maxp)
    ...
    def inference (self, y_, x0_=None):
        ...
        with tf.variable_scope (self._scope, reuse=True) as vs:
            for t in range (self._T):
                ...
                # wh_ = shrink_ss (uh_ + tf.matmul (W2_, res2_), tf.abs(theta2_), percent_dnn)
