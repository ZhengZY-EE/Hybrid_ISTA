...
class Hybrid_LISTA_cp_ComplexNet (LISTA_base):

    """
    Implementation of Hybrid learned ISTA with Complex Network.
    Used for dealing with large size data (32*32).
    """
    def __init__ (self, A, T, lam, untied, coord, scope, mode='D', CN_mode='DesNet', BN_flag = True, alpha_initial=0.0):
        """
        ...
        self._size= np.sqrt(self._N).astype(np.int32)
                ...
                if self._untied: # untied model
                    for j in range(self._T):
                        ...
                        paras_.append (tf.get_variable (name='pos_emb_'+str(j), 
                                                    shape=[1, 65, 64], dtype=tf.float32,
                                                    initializer=tf.orthogonal_initializer()))
                        ...
                        paras_.append (tf.get_variable (name='mlp2_'+str(j), 
                                                    shape=[128, 64], dtype=tf.float32,
                                                    initializer=tf.orthogonal_initializer()))
                        paras_.append (tf.get_variable (name='final_fc_'+str(j), 
                                                    shape=[65, 16], dtype=tf.float32,
                                                    initializer=tf.orthogonal_initializer()))
                        paras_total_.append(paras_)
                        paras_ = []
                ...
                if self._untied: # untied model
                    for j in range(self._T):
                        ...
                        paras_.append (tf.get_variable (name='fc_'+str(j), 
                                                    shape=[32*32, 32*32], dtype=tf.float32,
                                                    initializer=tf.orthogonal_initializer()))
                        paras_.append (tf.get_variable (name='dotmat_'+str(j), 
                                                    shape=[1, 32 * 32], dtype=tf.float32,
                                                    initializer=tf.orthogonal_initializer()))
                        paras_total_.append(paras_)
                        paras_ = []
    ...
    def setup_layers(self):
        ...
        #     self.vars_in_layer = list (zip (Ws1_, Ws2_, thetas1_, thetas2_, self.paras_, self.alphas_raw, self._bn_offs, self._bn_scas))
        # else:
    ...
    def inference (self, y_, x0_=None):
        ...
        with tf.variable_scope (self._scope, reuse=True) as vs:
            for t in range (self._T):
                ...
                # if self._CN == 'DesNet' or self._CN == 'UNet':  
                #     W1_, W2_, theta1_, theta2_, dnn_para_, alpha_raw, bn_offs, bn_scas = self.vars_in_layer [t]
                # else:
                ...
                if self._CN == 'DesNet':
                    ...
                    vh_0 = tf.reshape(vh_, [-1, self._size, self._size, 1])
                    batchsize = tf.shape(vh_0)[0]
                    vh_1 = tf.nn.conv2d_transpose(value=vh_0, filter=dnn_para_[0], output_shape=[batchsize, self._size*2, self._size*2, 16], strides=[1,2,2,1], padding='SAME')
                    ...
                    vh_0 = tf.reshape(vh_, [-1, self._size, self._size, 1])
                    batchsize = tf.shape(vh_0)[0]
                    vh_1 = tf.nn.conv2d_transpose(value=vh_0, filter=dnn_para_[0], output_shape=[batchsize, self._size*2, self._size*2, 8], strides=[1,2,2,1], padding='SAME')
                    vh_1 = relu_batch_normalization(vh_1, 0.0, 1.0)
                    # vh_1 = tf.nn.relu(tf.layers.batch_normalization(vh_1, self._BN_flag))
                    ...
                    vh_7 = tf.nn.conv2d_transpose(value=vh_4, filter=dnn_para_[6], output_shape=[batchsize, self._size, self._size, 16], strides=[1,2,2,1], padding='SAME')
                    vh_7 = relu_batch_normalization(vh_7, 0.0, 1.0)
                    # vh_7 = tf.nn.relu(tf.layers.batch_normalization(vh_7, self._BN_flag))

                    vh_8 = tf.nn.conv2d_transpose(value=vh_5+vh_7, filter=dnn_para_[7], output_shape=[batchsize, self._size*2, self._size*2, 8], strides=[1,2,2,1], padding='SAME')
                    ...
                    uh_ = tf.reshape(vh_9, tf.shape(vh_)) + vh_
                    ...
                    uh_ = tf.reshape(vh_1, tf.shape(vh_)) + vh_
                    
                elif self._CN == 'Fc':
                    
                    vh_0 = tf.reshape(vh_, [-1, self._size * self._size])
                    ...
                    uh_ = tf.reshape(vh_4, tf.shape(vh_)) + vh_
