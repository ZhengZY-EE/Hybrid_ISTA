...
class Hybrid_ISTA_ada_freeT_cs (LISTA_base):
    ...
    def setup_layers(self):
        ...
        # vars_in_layer [t]
        self.vars_in_layer = list (zip (D_inv_list[:-1], ts_scalar_ [:-1], self.paras_ [:-1], lams_[:-1]))
        self.vars_in_layer.append ((D_inv_list[-1], ts_scalar_ [-1], self.paras_ [-1], lams_[-1], self._vD_, ))
        self.ts_ = ts_scalar_
    ...
    def inference (self, y_, x0_=None):
        ...
        with tf.variable_scope (self._scope, reuse=True) as vs:
            for t in range (self._T):
                ...
                if t < self._T - 1:
                    D_inv_, t_, dnn_para_, lam = self.vars_in_layer [t]
                    D_ = self._kD_
                else:
                    D_inv_, t_, dnn_para_, lam, D_ = self.vars_in_layer [t]
                 
                # v_n
                res1_ = y_ - tf.matmul (self._kA_, xh_)
                vh_ = shrink_free (xh_ + tf.abs(t_) * tf.matmul (self._AT_, res1_), lam * tf.abs(t_))
                ...
                wh_ = shrink_free (uh_ + tf.abs(t_) * tf.matmul (self._AT_, res2_), lam * tf.abs(t_))
