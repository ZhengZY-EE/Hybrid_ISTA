...
class Hybrid_ISTA_cs (LISTA_base):

    def __init__ (self, Phi, D, T, lam, untied, coord, scope, mode='S', conv_num=3, kernel_size=3, feature_map=16, alpha_initial=0.0):
        """
    ...
    def setup_layers(self):
        ...
        with tf.variable_scope (self._scope, reuse=False) as vs:
            ...
            for t in range (self._T):
                ts1_scalar_.append (tf.get_variable (name="t1_scalar_%d"%(t+1),
                                                     dtype=tf.float32,
                                                     initializer=6.0))
                ts2_scalar_.append (tf.get_variable (name="t2_scalar_%d"%(t+1),
                                                     dtype=tf.float32,
                                                     initializer=6.0))

                #######################################################
                lam1_.append(tf.get_variable (name="lam1_%d"%(t+1),
                                              dtype=tf.float32,
                                              initializer=self._lam))
                lam2_.append(tf.get_variable (name="lam2_%d"%(t+1),
                                              dtype=tf.float32,
                                              initializer=self._lam))
        ...
        # vars_in_layer [t]
        self.vars_in_layer = list (zip (D_inv_list[:-1], ts1_scalar_ [:-1], ts2_scalar_ [:-1], self.paras_ [:-1], self.alphas_raw [:-1], lam1_[:-1], lam2_[:-1]))
        self.vars_in_layer.append ((D_inv_list[-1], ts1_scalar_ [-1], ts2_scalar_ [-1], self.paras_ [-1], self.alphas_raw [-1], lam1_[-1], lam2_[-1], self._vD_, ))
        self.ts1_ = ts1_scalar_
        self.ts2_ = ts2_scalar_
    ...
    def inference (self, y_, x0_=None):
        ...
        with tf.variable_scope (self._scope, reuse=True) as vs:
            for t in range (self._T):
                ...
                if t < self._T - 1:
                    D_inv_, t1_, t2_, dnn_para_, alpha_raw, lam1, lam2 = self.vars_in_layer [t]
                    D_ = self._kD_
                else:
                    D_inv_, t1_, t2_, dnn_para_, alpha_raw, lam1, lam2, D_ = self.vars_in_layer [t]

                t1_ = tf.sigmoid(t1_) * self._theta
                t2_ = tf.sigmoid(t2_) * t1_/2
                 
                # v_n
                res1_ = y_ - tf.matmul (self._kA_, xh_)
                vh_ = shrink_free (xh_ + tf.abs(t1_) * tf.matmul (self._AT_, res1_), lam1 * tf.abs(t1_))
                ...
                wh_ = shrink_free (uh_ + tf.abs(t2_) * tf.matmul (self._AT_, res2_), lam2 * tf.abs(t2_))

                # 1-alpha_n (0, 1 - lower bound of alpha)
                tu_x = tf.abs(t1_) * tf.square(tf.norm(uh_-xh_))
                tv_x = tf.abs(t2_) * tf.square(tf.norm(vh_-xh_))
