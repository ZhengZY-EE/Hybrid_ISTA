...
def setup_sc_training (model, y_, x_, y_val_, x_val_, x0_,
    ...
    for t in range (model._T):
        ...
        var_list = []
        for var in model.vars_in_layer [t]:
            if isinstance(var, list):
                for vv in var:
                    if vv not in train_vars:
                        var_list.append(vv)
            else:
...
def setup_sc_training_HELISTA (model, y_, x_, y_val_, x_val_, x0_,
                       init_lr, decay_rate, lr_decay):
    """TODO: Docstring for setup_training.
...
def setup_cs_training (model, y_, f_, y_val_, f_val_, x0_,
                       init_lr, decay_rate, lr_decay, lasso_lam = 0.0):
    """TODO: Docstring for setup_training.
...
def setup_cs_training_ISTANet_Plus (model, y_, f_, y_val_, f_val_, x0_,
                                    init_lr, decay_rate, lr_decay, lasso_lam = 0.0):

    xhs_, fhs_, resi_error_ = model.inference (y_, x0_)
    xhs_val_, fhs_val_, resi_error_val_ = model.inference (y_val_, x0_)
...
def setup_cs_training_pure_ISTANet_Plus (model, y_, f_, y_val_, f_val_, x0_,
                                        init_lr, decay_rate, lr_decay, lasso_lam = 0.0):

    fhs_, resi_error_ = model.inference (y_, x0_)
    fhs_val_, resi_error_val_ = model.inference (y_val_, x0_)
    ...
    for t in range (model._T):
        ...
        l2_   = tf.nn.l2_loss (fhs_ [t+1] - f_)
        resi_ = tf.reduce_mean(tf.square(resi_error_[t]))
        loss_ = l2_ + 0.01 * resi_
        # loss_ = l2_ 
        nmse_ = l2_ / nmse_denom_
        # validation
        l2_val_   = tf.nn.l2_loss (fhs_val_ [t+1] - f_val_)
        resi_val_ = tf.reduce_mean(tf.square(resi_error_val_[t]))
        loss_val_ = l2_val_ + 0.01 * resi_val_
        # loss_val_ = l2_val_
        nmse_val_ = l2_val_ / nmse_denom_val_
        ...
        # op_ = tf.train.AdamOptimizer (init_lr).minimize (loss_, var_list=var_list)
        # training_stages.append ((layer_info, loss_, nmse_,
        #                         loss_val_, nmse_val_, op_, var_list))
...
def do_training (sess, stages, savefn, scope, val_step, maxit, better_wait):
    ...
    """
    if not savefn.endswith(".txt"):
        savefn_txt = savefn + ".txt"
    if not savefn.endswith(".npz"):
        savefn += ".npz"
    if os.path.exists(savefn):
        content = 'Pretrained model found. Loading...\n'
        sys.stdout.write(content)
        write_txt(savefn_txt, content)
    ...
    # for s in stages:
        # name, loss_, nmse_, loss_val_, nmse_val_, op_ = s['name'], s['loss_'], s['nmse_'], s['loss_val_'], s['nmse_val_'], s['op_']

        """Skip stage done already."""

        if name in done:
            content = 'Already did {}. Skipping\n'.format(name)
            sys.stdout.write(content)
            write_txt(savefn_txt, content)
            continue
        ...
        print (name + ' ' + var_disc)
        write_txt(savefn_txt, name + ' ' + var_disc)
        ...
        for i in range (maxit+1):
            ...
            if i % val_step == 0:
                ...
                if i % (100 * val_step) == 0:
                    print('')
                    write_txt(savefn_txt, "\r| i={i:<7d} | loss_tr={loss_tr:.6f} | "
                        "db_tr={db_tr:.6f}dB | loss_val ={loss_val:.6f} | "
                        "db_val={db_val:.6f}dB | (best={db_best_val:.6f})\n"\
                            .format(i=i, loss_tr=loss_tr, db_tr=db_tr,
                                    loss_val=loss_val, db_val=db_val,
