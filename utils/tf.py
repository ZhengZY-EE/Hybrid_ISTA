...
def MT(r_, theta_, theta_bar_):
    """
    Implementation of multistage thresholding operator.
    """
    theta_ = tf.abs(theta_)
    theta_bar_ = tf.abs(theta_bar_)
    theta_ratio = theta_bar_ / (theta_bar_ - theta_)
    x1_ = theta_ratio * tf.sign(r_) * tf.maximum( tf.abs(r_) - theta_, 0.0 )
    flag = tf.sign( tf.abs(r_) - theta_bar_)
    xout = (flag + 1) * r_ / 2 - (flag - 1) * x1_ / 2 
    return xout
